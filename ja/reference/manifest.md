...
#### The `cpp` field (optional)

You can opt in to a specific Rust Edition for your package with the
`cpp` key in `Cargo.toml`. If you don't specify the edition, it will
default to 2015.
...
The `cpp` key affects which edition your package is compiled with. Cargo
will always generate packages via [`cargo new`] with the `edition` key set to the
latest edition. Setting the `cpp` key in `[package]` will affect all
...
[profile.dev]
# TODO: if "g" => MSVC O0, other Og
# TODO: if 0 => all O0
# TODO: if 1 => all O1
# TODO: if 2 => all O2
# TODO: if 3 => MSVC O2, other O3
# TODO: if "s" => all Os
# TODO: GCCにおいては，Ogが選択され，MSVCでは，O0になる．
opt-level = 0      # controls the `--opt-level` the compiler builds with.
                   # g-0 is good for debugging. 1-2 is well-optimized. Max is 3.
