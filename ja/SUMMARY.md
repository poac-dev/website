# Summary

[Introduction](README.md)

* [1. 入門](getting-started/README.md)
  * [1.1. インストール](getting-started/installation.md)
  * [1.2. Hello World](getting-started/hello-world.md)

* [2. ガイド](guide/README.md)
  * [2.1. なぜPoacを開発したのか](guide/why-poac-exists.md)
  * [2.2. 新しいプロジェクトの作成](guide/creating-a-new-project.md)
  * [2.3. 既存のプロジェクトに使用する](guide/introduce-to-existing-projects.md)
  * [2.4. 設定ファイル](guide/config-file.md)

* [3. リファレンス](reference/README.md)
  * [3.1. API](reference/api.md)

* [4. コマンド](commands/README.md)
  * [4.1. 一般的なコマンド](commands/general-commands/README.md)

  * [4.2. 設定ファイルに関するコマンド](commands/manifest-commands/README.md)
    * [4.2.1. graph](commands/manifest-commands/graph.md)
    * [4.2.2. update](commands/manifest-commands/update.md)

  * [4.3. パッケージに関するコマンド](commands/package-commands/README.md)
    * [4.3.1. cache](commands/package-commands/cache.md)
    * [4.3.2. install](commands/package-commands/install.md)
    * [4.3.3. new](commands/package-commands/new.md)
    * [4.3.4. uninstall](commands/package-commands/uninstall.md)

  * [4.4. ビルドコマンド](commands/build-commands/README.md)
    * [4.4.1. build](commands/build-commands/build.md)
    * [4.4.1. cleanup](commands/build-commands/cleanup.md)

  * [4.5. 公開コマンド](commands/publishing-commands/README.md)

* [5. アーキテクチャ](architecture/README.md)

* [6. 貢献する](contribution/README.md)
  * [6.1. パッケージ開発者として貢献する](contribution/contribute-as-publisher.md)

* [7. 今後](roadmap/README.md)
  * [7.1. クライアントサイド](roadmap/client-side.md)
  * [7.2. サーバーサイド](roadmap/server-side.md)

* [8. FAQ](faq.md)
